...
class DooModel{
    ...
    public static function _limit($limit=1, $asc='', $desc='', $options=null){
        if($asc!='' || $desc!='' || $options!==null){
            ...
            return self::db()->find(self::$className, $options);
        }
        return self::db()->find(self::$className, array('limit'=>$limit));
    }
    ...
    public static function _getOne($model=null, $options=null){
        ...
        if($options!==null){
            $options['limit'] = 1;
            return self::db()->find($model, $options);
        }
        return self::db()->find($model, array('limit'=>1));
    }
