    ...
    public function auto_connect($subfolder='/', $autoroute_alias=null){
        ...
        if(empty($uri[1])===false){
			...
			if( strpos($method_name, '-') !== false ){
                ...
                $methodNameArray = str_split($method_name);
                $methodNameArray[0] = strtolower($methodNameArray[0]);
                $method_name = implode('',$methodNameArray);
			}
        ...
        if($autoroute_alias!==null){
            ...
            if(isset($autoroute_alias[$alias])===true){
                ...
                $convertnameArray = str_split($convertname);
				$convertnameArray[0] = strtolower($convertnameArray[0]);
				$convertname = implode('',$convertnameArray);
