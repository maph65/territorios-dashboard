...
class CatalogosController extends SecurityController {
    ...
    public function index() {
        ...
        if (!self::validarPermisos('catalogos')) {
        ...
        if (!empty($categoria)) {
            ...
            $this->data['location'] = 'catalogos';
            $this->renderc('admin/catalogos', $this->data);
        } else {
            header('location:' . Doo::conf()->APP_URL . 'home?error=1');
        }
    }

    public function publicarCatalogos() {
        session_start();
        if (!self::validarSesion()) {
            header('location:' . Doo::conf()->APP_URL . '?error=2');
            session_destroy();
        ...
        if (isset($_POST['titulo']) && !empty($_POST['titulo']) && isset($_POST['descripcion']) && !empty($_POST['descripcion']) && isset($_POST['url']) && !empty($_POST['url']) && isset($_POST['viewport-x']) && !empty($_POST['viewport-x']) && isset($_POST['viewport-y']) && !empty($_POST['viewport-y']) && isset($_FILES['imagen']) && !empty($_FILES['imagen']
                )
        ) {
            $categoria = new CtCategoriaDescarga();
            $categoria->slug = 'catalogos';
                ...
                if (!filter_var($url, FILTER_VALIDATE_URL) === false) {
                    ...
                    if ($gd->checkImageExtension('imagen')) {
                        ...
                        if (empty($val)) {
                            ...
                            if ($result) {
                                header('location:' . Doo::conf()->APP_URL . 'catalogos?success=1');
                            } else {
                                header('location:' . Doo::conf()->APP_URL . 'catalogos?error=6');
                            }
                        } else {
                            //print_r($notificacion->validate());
                            header('location:' . Doo::conf()->APP_URL . 'catalogos?error=5');
                        }
                    } else {
                        header('location:' . Doo::conf()->APP_URL . 'catalogos?error=4');
                    }
                } else {
                    header('location:' . Doo::conf()->APP_URL . 'catalogos?error=3');
                }
            } else {
                header('location:' . Doo::conf()->APP_URL . 'catalogos?error=2');
            }
        } else {
            header('location:' . Doo::conf()->APP_URL . 'catalogos?error=1');
        }
    }
    ...
    public function editarCatalogo() {
        ...
        if (!empty($categoria) && !empty($idDescarga)) {
            ...
            if (!empty($descarga)) {
                ...
                $this->data['location'] = 'catalogos';
                $this->renderc('admin/editar/catalogos', $this->data);
            } else {
                header('location:' . Doo::conf()->APP_URL . 'catalogos?error=6');
            }
        } else {
            ...
            if (!empty($descarga)) {
                ...
                if (!filter_var($url, FILTER_VALIDATE_URL) === false) {
                    ...
                    if (empty($val)) {
                        ...
                        if ($result) {
                            header('location:' . Doo::conf()->APP_URL . 'catalogos?success=3');
                        } else {
                            header('location:' . Doo::conf()->APP_URL . 'catalogos?error=6&result');
                        }
                    } else {
                        header('location:' . Doo::conf()->APP_URL . 'catalogos?error=6&empty');
                    }
                } else {
                    header('location:' . Doo::conf()->APP_URL . 'catalogos?error=3');
                }
            } else {
                header('location:' . Doo::conf()->APP_URL . 'catalogos?error=6&notfound');
            }
        } else {
            header('location:' . Doo::conf()->APP_URL . 'catalogos?error=2');
        }
    ...
    public function eliminarCatalogo() {
        ...
        if ($idDescarga) {
            ...
            if (!empty($d)) {
                ...
                $d->delete();
                header('location:' . Doo::conf()->APP_URL . 'catalogos?success=2');
            } else {
                header('location:' . Doo::conf()->APP_URL . 'catalogos?error=5');
            }
        } else {
    ...
    public function obtenerCatalogo() {
        session_start();
        $resultado = array();
        if (self::validarSesion() && self::validarPermisos('catalogos')) {
            Doo::loadModel('CtCategoriaDescarga');
            Doo::loadModel('CtDescarga');
            if (isset($this->params['iddescarga'])) {
                ...
                $descarga->id_descarga = $idDescarga;
                //$descarga = $descarga->getOne();
                $descarga = $descarga->relate('CtCategoriaDescarga', array('where' => 'ct_categoria_descarga.slug = "catalogos"', 'limit' => 1));
                if (!empty($descarga)) {
                    $resultado['status'] = 'ok';
                    $arrayDescarga = array();
    ...
    public function obtenerCatalogos() {
        ...
        $descarga = new CtDescarga();
        $descargas = $descarga->relate('CtCategoriaDescarga', array('where' => 'ct_categoria_descarga.slug = "catalogos"', 'desc' => 'fecha_alta', 'limit' => 30));
